...
class DbHandler
  ...
  def fetch_data
    @regular = Database::DB.fetch("select url, count(*) from webserver_logs group by(webserver_logs.url) order by count(webserver_logs.url) desc;").map([:url, :count])
    @unique = Database::DB.fetch("select url, count(distinct(url, ip)) from webserver_logs group by(webserver_logs.url);").map([:url, :count]).sort_by {|k, v| v }.reverse
